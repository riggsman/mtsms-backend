...
def create_complaint(db: Session, complaint: ComplaintRequest, institution_id: int | None = None, current_user: User | None = None) -> Complaint:
    """Create a new complaint"""
    final_institution_id = institution_id or (current_user.institution_id if current_user else None)
    if not final_institution_id:
        raise ValidationError("institution_id is required to create a complaint")

    # Store screenshots as JSON array of file URLs only (files are saved to filesystem)
...
def get_complaint(db: Session, complaint_id: int, institution_id: int | None = None) -> Complaint:
...
def delete_complaint(db: Session, complaint_id: int, institution_id: int | None = None) -> bool:
...
def get_student_complaints(db: Session, student_id: str, institution_id: int | None = None) -> List[Complaint]:
